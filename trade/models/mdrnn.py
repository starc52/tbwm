...
class _MDRNNBase(nn.Module):
    def __init__(self, latents, actions, hiddens, gaussians):
        ...
        self.gmm_linear = nn.Linear(
            hiddens, (2 * latents + 1) * gaussians + 2) # input size = hidden size, output size = (2*LSIZE+1)*#gaussians + 2 why? each gaussian outputs mu, sigma and a probability of the data coming from a particular gaussian. next, the 2 is added for rewards and dones.
...
class MDRNNCell(_MDRNNBase):
    ...
    def forward(self, action, latent, hidden): # pylint: disable=arguments-differ
        ...
        out_rnn = next_hidden[0] # here [0] is the difference
