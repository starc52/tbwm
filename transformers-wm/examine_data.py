...
def plot_rollout():
    ...
    # setting up subplots
    plt.subplot(2, 2, 1)
    monitor_obs = plt.imshow(np.zeros((96, 96, 3)))
    plt.subplot(2, 2, 2)
    monitor_next_obs = plt.imshow(np.zeros((96, 96, 3)))
    plt.subplot(2, 2, 3)
    monitor_diff = plt.imshow(np.zeros((96, 96, 3)))

    for data in dataloader:
        obs_seq = data[0].numpy().squeeze().astype(np.uint8)
        action_seq = data[1].numpy().squeeze()
        next_obs_seq = data[-1].numpy().squeeze().astype(np.uint8)
