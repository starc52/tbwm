...
from models.transformers_de import TransformersDEXL, TransformersDE, gmm_loss

parser = argparse.ArgumentParser("TransformersDE training")
...
# Loading model
transformers_de_dir = join(args.logdir, 'transformers_de') # here
transformers_de_file = join(transformers_de_dir, 'best.tar')

if not exists(transformers_de_dir):
    mkdir(transformers_de_dir)

transformers_de = TransformersDE(LSIZE, ASIZE, RSIZE, 5) # here
transformers_de.to(device)
optimizer = torch.optim.Adam(transformers_de.parameters(), lr=1e-4, betas=(0.9, 0.98), eps=1e-9)
scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.5, patience=5)
earlystopping = EarlyStopping('min', patience=30)


if exists(transformers_de_file) and not args.noreload:
    transformers_de_state = torch.load(transformers_de_file)
    print("Loading TransformersDE at epoch {} "
          "with test error {}".format(
              transformers_de_state["epoch"], transformers_de_state["precision"]))
    transformers_de.load_state_dict(transformers_de_state["state_dict"])
    optimizer.load_state_dict(transformers_de_state["optimizer"])
...
def to_latent(obs, next_obs):
    ...
    :args obs: 5D torch tensor (BSIZE, SEQ_LEN, 3, SIZE, SIZE) was ASIZE instead of 3 before. Doesn't make sense with ASIZE.
    :args next_obs: 5D torch tensor (BSIZE, SEQ_LEN, 3, SIZE, SIZE) was ASIZE instead of 3 before.
    ...
    latent_obs, action,\
        ...
        latent_next_obs = [arr.transpose(1, 0)
                           for arr in [latent_obs, action,
                                       reward, terminal,
                                       latent_next_obs]]
    mus, sigmas, logpi, rs, ds, _ = transformers_de(action, latent_obs) #what are logpi, rs, ds
...
def data_pass(epoch, train, include_reward): # pylint: disable=too-many-locals
    ...
    if train:
        transformers_de.train()
        loader = train_loader
    else:
        transformers_de.eval()
        loader = test_loader
...
for e in range(epochs):
    ...
    if is_best:
        cur_best = test_loss
    checkpoint_fname = join(transformers_de_dir, 'checkpoint.tar')
    save_checkpoint({
        "state_dict": transformers_de.state_dict(),
        "optimizer": optimizer.state_dict(),
        'scheduler': scheduler.state_dict(),
        'earlystopping': earlystopping.state_dict(),
        "precision": test_loss,
        "epoch": e}, is_best, checkpoint_fname,
                    transformers_de_file)
